...
const { validateLevelDefinition, LEVELS } = await import('../registry.js');
...
test('level validation rejects impossible definitions', () => {
  assert.equal(validateLevelDefinition(LEVELS[0]).valid, true);
...
test('level validation rejects unsafe cell payloads and values', () => {
  const layoutLevel = JSON.parse(JSON.stringify(LEVELS[0]));
  ...
  const chanceLevel = JSON.parse(JSON.stringify(LEVELS[2]));
